...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["http://localhost", "http://localhost:4200"],  # In production, restrict to your domain
...
9. Para Pressão Arterial (PA), reconheça formatos como "150 por 95", "150/95" ou "150 e 95" como sistólica/diastólica.
...

{
  ...
  "jsonUniversal": {
    "HDA_Tecnica": "Sintetize as queixas do paciente em uma narrativa técnica profissional. Não use aspas ou 'o paciente disse'. Exemplo: 'Paciente relata cefaleia holocraniana de forte intensidade há 3 dias, sem melhora com analgésicos comuns. Apresenta antecedentes de DM2 em uso de metformina'.",
...
async def process_with_audio(
    ...
    idade: Optional[int] = Form(default=None),
):
    """
    ...
    try:
        ...
        user_message = _build_user_prompt(transcript, context, cenario, str(idade) if idade else "")
...
async def process_text_only(
    ...
    try:
        ...
        user_message = _build_user_prompt(texto_transcrito, context, cenario, str(idade) if idade else "")
